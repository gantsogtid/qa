...
const BAR    = ['#eab308', '#64748b', '#f97316', P, P]
    ...
    <div style={{ minHeight: '100vh', background: '#f0f4f4' }}>
      ...
      <div style={{ maxWidth: 600, margin: '-1.5rem auto 0', padding: '0 1rem 1rem' }}>
        <div style={{ display: 'flex', gap: 8, marginBottom: '1.25rem' }}>
          {[
            { n: total, label: 'Нийт асуулт', color: P },
            { n: totalLikes, label: 'Нийт санал', color: PD },
            { n: top.length, label: 'Топ асуулт', color: '#7c3aed' },
          ].map(s => (
            <div key={s.label} style={{ flex: 1, background: '#fff', borderRadius: 16, padding: '12px 8px', textAlign: 'center', boxShadow: '0 4px 16px rgba(0,0,0,.07)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {top.map((q, i) => (
            <div key={q.id} style={{ background: BG[i], border: `2px solid ${BORDER[i]}`, borderRadius: 16, padding: '1rem 1.25rem', boxShadow: i === 0 ? '0 4px 20px rgba(234,179,8,.15)' : '0 2px 8px rgba(0,0,0,.04)' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, marginBottom: 10 }}>
                <span style={{ fontSize: 30, flexShrink: 0, lineHeight: 1 }}>{MEDALS[i]}</span>
                <p style={{ flex: 1, fontSize: 15, fontWeight: i < 3 ? 600 : 400, lineHeight: 1.6, color: '#1e293b' }}>{q.text}</p>
                <div style={{ flexShrink: 0, textAlign: 'center', background: '#fff', borderRadius: 10, padding: '8px 14px', border: `1px solid ${BORDER[i]}`, boxShadow: '0 1px 4px rgba(0,0,0,.06)' }}>
                  <div style={{ fontSize: 22, fontWeight: 800, color: P }}>{q.likes}</div>
                  <div style={{ fontSize: 10, color: '#94a3b8' }}>санал</div>
                </div>
              </div>
              <div style={{ height: 6, background: 'rgba(0,0,0,.06)', borderRadius: 4, overflow: 'hidden' }}>
                <div style={{ height: '100%', width: `${Math.round((q.likes / maxLikes) * 100)}%`, background: BAR[i], borderRadius: 4, transition: 'width .6s ease' }} />
              </div>
            </div>
          ))}
        </div>

        <p style={{ textAlign: 'center', fontSize: 12, color: '#94a3b8', marginTop: '1.5rem' }}>↻ Шинэчлэлт автоматаар харагдана</p>
      </div>
    </div>
  )
}
