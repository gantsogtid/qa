...
import { supabase, getSetting, type AttendanceRow } from '@/lib/supabase'
...
export default function CertificatePage() {
  ...
  async function search() {
    ...
    // Бүх сургалтаас checked_in=true хайх
    ...
    // Нэг л сургалт байвал шууд үүсгэх
  ...
  async function drawCertificate(person: Match) {
    ...
    await new Promise<void>(resolve => {
      ...
      img.onload = () => {
        ...
        ctx.fillText(
          displayName(person),
          canvas.width  * (certNameX.current / 100),
          canvas.height * (certNameY.current / 100),
        )
    ...
    <div style={{ minHeight: '100vh', background: '#f0f4f4' }}>
      ...
      <div style={{ maxWidth: 480, margin: '-1.5rem auto 0', padding: '0 1rem 6rem' }}>
        ...
        <div style={{ background: '#fff', borderRadius: 20, padding: '1.25rem', boxShadow: '0 8px 32px rgba(0,0,0,.1)', marginBottom: 14 }}>
          ...
          <button
            onClick={search}
            disabled={loading || settingsLoading || !phone.trim()}
            style={{
              width: '100%', padding: '13px',
              background: phone.trim() && !settingsLoading ? GRAD : '#e2e8f0',
              color: phone.trim() && !settingsLoading ? '#fff' : '#94a3b8',
              borderRadius: 12, fontSize: 15, fontWeight: 700,
            }}
          >
            {loading ? 'Хайж байна...' : settingsLoading ? 'Ачааллаж байна...' : 'Гэрчилгээ авах'}
          </button>
        </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {matches.map((m, i) => (
                <button key={i} onClick={() => pickEvent(m)}
                  ...
                  {m.eventInfo?.event_date && (
                    <div style={{ fontSize: 12, color: '#64748b', marginTop: 2 }}>{m.eventInfo.event_date}</div>
                  )}
                </button>
              ))}
            </div>
        ...
        {selected && !certReady && !loading && !certTemplateUrl.current && (
          <div style={{ background: '#fff', borderRadius: 16, padding: '1.5rem', textAlign: 'center', boxShadow: '0 2px 8px rgba(0,0,0,.08)' }}>
